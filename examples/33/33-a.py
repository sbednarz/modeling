...
def model(y, t):
    ...
    #rate constants
    k1 = 1e-3*3600
    k2 = 0 # 1e-4*3600 # <--- change the value, see Y & S
    # ODEs
    dAdt = -k1*A*B
    dBdt = -k1*A*B
    dCdt = k1*A*B - k2*B*C
...
plt.savefig('33-1c.pdf')
plt.close()
#
fig,ax1 = plt.subplots()
ax1.set_title('Fig. 33-1d.pdf')
ax1.plot(t , alpha, 'b--', label='Conv.A')
ax1.plot(t ,Y, 'g--',  label='Yield C')
ax1.plot(t ,S-0.5, 'm--',  label='Sel.C') #S-0.5 just to make more visible on plot
ax1.set_xlabel('Reaction time, h')
ax1.set_ylabel('%')
ax1.legend(loc='upper right', ncol=3, handletextpad=0, 
        columnspacing=0.4)
plt.savefig('33-1d.pdf')
plt.close()
# TODO
# validation print A+B+...
